...
/**
 * Detecte si un joueur est affamee
 * @param pos la position de jeux
 * @param joueur le joueur dont on veut tester s'il est affamee
 * @return joueur affamee ou non
 */
...
/**
 * Recopie d'une position dans une autre
...
/**
 * Joue un coup
 * @param cs les cases du jeux
 * @param newPos la nouvelle position avec le coup jouee
 * @param pos l'ancienne position avant que le coup soit joue
...
int valeur_minimaxAB(CaseSuivante* cs, Position* pos,const int joueur, int alpha,int beta, const int pmax, const bool gagne, std::unordered_map<Board,int> hashScoresPositions);

int calculer_coup(CaseSuivante* cs, Position* pos,const int joueur, int alpha, int beta, const int pmax,const bool gagne, std::unordered_map<Board,int> hashScoresPositions){
	Position newPos;
	if (joueur==0){// MAX
		//cout << "MAX";
		for(int i=0;i<6;i++){
			if (jouer_coup(cs,&newPos,pos,joueur,i)){
				int val;
				std::unordered_map<Board,int>::iterator it = hashScoresPositions.find(newPos._Cases);
...
int decisionAB(CaseSuivante* cs,Position* pos,int pmax, bool gagne){
	// on regarde le nombre de case vide et on ajoute de la profondeur eventuellement
	// k case vide = profmax * k/12
	std::unordered_map<Board,int> hashScoresPositions;
	int k=0;
	for(int i=0;i<6;i++){
		if (pos->_Cases[i] == 0) k++;
