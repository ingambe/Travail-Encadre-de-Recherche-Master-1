...
gcc<-read.table(file = "gcc/resultat_6.txt",sep="\n", header=FALSE)
clang<-read.table(file="clang/resultat_6.txt",sep="\n", header=FALSE)
intel<-read.table(file="intel/resultat_6.txt",sep="\n", header=FALSE)
...
legend("bottomleft", generalDetails, bty = "n", cex=0.8)
